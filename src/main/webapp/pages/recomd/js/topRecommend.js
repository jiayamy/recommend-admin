...
function initGridTable() {
	...
	$(grid_selector).jqGrid({
		//direction: "rtl",
        url: webroot + "recomd/listTopRecommend.msp",
		datatype: "json",
		/*height: 350,*/
		colNames:["序号", "节目ID", "节目名称", "所属产品", "状态",
		          "创建者", "创建时间", "修改者", "修改时间"
		],
		colModel:[
    	...
    	Lobibox.confirm({
    		...
    		callback: function ($this, type, eve) {
    			if(type == "yes"){
    				...
    				var data = {"ids":ids};
    				$.post(webroot+"recomd/delTopRecommend.msp",
    					data,
    					function(data){
...
function editSaveTopRecommend() {
	...
	$.ajax({
		type:"post",
		url:webroot+"recomd/editTopRecommend.msp",
		data:{
			"id": id,			
			"prdType": prdType, 
			"topId":topId,			 
...
function addTopRecommend() {
    $("#addTopRecommendModal").modal("show"); 
}
