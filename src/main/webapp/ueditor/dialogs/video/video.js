...
(function(){
    ...
    function initMamVideo(){
    	var _this = this;
    	var vdoSct = document.getElementById('videoUrl');
    	
    	vdoSct.onchange=function(){
    		
    		createPreviewVideo( vdoSct.options[vdoSct.selectedIndex].value );
    	}
    	
        if(editor.options.assetId) {
        	...
        	if(!this.isLoadingData) {
		        ...
		        ajax.request(url, {
		            ...
		            'data': utils.extend({
		                    ...
		                    size: this.listSize,
		                    aid: $(editor.options.aid, window.parent.document).val()||'',
		                    assetId: $(editor.options.assetId, window.parent.document).val()||''
		            ...
		            'onsuccess': function (r) {
		                try {
		                    ...
		                    if (json.state == 'SUCCESS') {
		                        ...
		                        if(json.list) {
		                        	vdoSct.options.add(new Option('--  请选择   --', ''));
		                        	for(var i=0;i<json.list.length;i++) {
		                        		var vdo = json.list[i];
		                        		vdoSct.options.add(new Option(vdo.name, vdo.url));
		                        	}
		                        }
		                    }
    ...
    function createPreviewVideo(url){
        ...
        '</embed>'*/
        '<embed class="previewVideo" src="' + conUrl + '"' + 
	        'width="420"' + 
	        'height="280"' + 
